...
```{r}
toolik_chlorophyll <- read.csv(here::here("data", "data-raw", "toolik_chlorophyll.csv")) #Read in data
```
```{r}
...
toolik_chlorophyll$active_chl_a_ug_l <- as.numeric(toolik_chlorophyll$active_chl_a_u)
```
...
ggplot(data = toolik_chlorophyll,
  ...
  geom_point(aes(col = active_chl_a_ug_l), show.legend = FALSE) +
  scale_color_gradient(low = "blue", high = "orange") + 
  theme_minimal() +
  labs(title = "Toolik Chlorophyll Concentration",
       x = "Depth (meters)",
